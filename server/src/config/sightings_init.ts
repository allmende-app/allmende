...
export const initializeLocations = async () => {
    ...
    const steps = sightings.filter(s => s.lat && s.lng && !s.location && !s.subname);
    for (let i = 0; i < steps.length; i += 20) {
        const elements = steps.slice(i, i + 20);
        for (let j = 0; j < elements.length; j += 5) {
            const locations = await Promise.all(elements.slice(j, j + 5).map(e => reverseLocationSearch(e.lng as number, e.lat as number)));
            await Promise.all(locations.map((l, index) => {
