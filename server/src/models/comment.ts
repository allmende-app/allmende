...
export interface ICommentModel extends Model<ICommentDocument> {
    ...
    findCommentByIDAndDelete: (id: string, user: ObjectId | string) => Promise<ICommentDocument>;
    findCommentByIDAndEdit: (id: string, user: ObjectId | string, comment: CommentInput) => Promise<ICommentDocument>;
}
...
commentSchema.statics.findCommentByIDAndEdit = async function(id: string, user: ObjectId | string, comment: CommentInput) {
...
commentSchema.statics.findCommentByIDAndDelete = async function(id: string, user: ObjectId | string) {
    const doc = await this.findById(id);
    if (doc.author == user) {
        await doc.delete();
        return doc;
